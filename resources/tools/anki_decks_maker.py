...
def vocab_type_sort_key(path_name):
    ...
    student would encounter the sections in the textbooks.
    """
                ...
                if filter_regex.search(html) == None: # Filter out exercise types that are NOT vocab
                    ...
                    for jp, eng in vocab.items():
                        ...
                        note = genanki.Note(model=my_model,
                                            fields=[re.sub(r'(.*?)\|(.*?)$', r'<ruby>\1<rt>\2</rt></ruby>', jp) + '<div class="sentence">' + sentence + '</div>', english],
                                            tags=['Quartet', f'lesson-{lesson_number}', *tags])

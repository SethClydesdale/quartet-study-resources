...
filter_regex = re.compile(r"漢字の練習：読み方|漢字の練習：意味")
...
def lesson_sort_key(path_name):
    ...
    student would encounter the lessons in the Quartet books.
    """
...
def vocab_type_sort_key(path_name):
    ...
    student would encounter the sections in the Quartet books.
    """
    match_groups = re.match(r'^(.*)-(\d\d*)$', path_name).groups()
...
.nightMode.card {
  ...
  background-color:#222;
    ...
    for lesson_folder in sorted(lessons_folder.glob('lesson*'),
        ...
        lesson_dict = {}
        for vocab_folder in sorted(chain(lesson_folder.glob('kanji*')),
                                   key=lambda path: vocab_type_sort_key(path.name)):
            with open(vocab_folder.joinpath('index.html'), 'r', encoding='UTF8') as f:
                html = f.read()
            for pathpart in ["kanji-1\\", "kanji-2\\"]: # \\ may need to be changed to / to properly execute, depending on your OS
        ...
        for k,v in lesson_dict.items():
            ...
            note = genanki.Note(model=my_model,
                                    ...
                                    tags=['Quartet', f'lesson-{lesson_number}', "Kanji"])
    ...
    print('All Anki decks have been generated!')

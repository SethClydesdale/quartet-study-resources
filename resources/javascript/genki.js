...
(function (window, document) {
  ...
  var Genki = {
    ...
    // remnant from Genki Study Resources, but I'll leave this functionality just in case a new edition of Quartet comes out and I feel like adapting the resources
    ...
    generateQuiz : function (o) {
      ...
      *********************************/
      ...
      if (typeof o.type === 'object') {
        ...
        // example: https://sethclydesdale.github.io/quartet-study-resources/lessons/lesson-1/vocab-1/?begin=0 (this starts multiple choice, 1 would start drag and drop)
        ...
        modal = GenkiModal.open({
          ...
          '<div class="center">'+
            (/\/vocabulary-index\/|\/custom-vocab\//.test(window.location) ? '' : '<div>'+
              ...
              document.title.replace(/ \| Quartet Study Resources.*$/, '')+
            '</div><br>')+
        ...
        zone.innerHTML = '<div id="quiz-info">' + o.info + '<br><span class="en">If you don\'t know how to type in Japanese on your computer, please visit our help page by <a href="../../../help/writing/' + Genki.local + '" target="_blank">clicking here</a>.</span><span class="ja">パソコンで日本語を入力する方法がわからない場合は、<a href="../../../help/writing/' + Genki.local + '" target="_blank">ヘルプページ</a>をご覧ください。</span></div><div class="text-block">' + o.quizlet.replace(/\{.*?\}/g, function (match) {
          ...
          if (data[0] == '!IMG') {
            ...
            width =
              ...
              flag[3] && /width:/.test(flag[3]) ? flag[3].split(':')[1] :
            ...
            return '<span class="writing-zone">'+
              ...
              '>'+
              ((hint && !/answer|furigana|placeholder/.test(flag[0]) || flag[1] && /hint:/.test(flag[1]) || flag[2] && /hint:/.test(flag[2])) ? '<span class="problem-hint' + ((flag[0] && /spoiler/.test(flag[0]) || flag[2] && /spoiler/.test(flag[2]) || flag[3] && /spoiler/.test(flag[3])) ? ' spoiler' : '') + '">' + (
                ...
                if (/^!I/.test($1)) {
                  var spacer = '／', alpha = false;
                  
                  if (/spacer/.test(flag[0])) {
                    alpha = flag[0].split(':')[1] == 'alpha';
                    spacer = alpha ? '' : flag[0].split(':')[1];
                  }
                  ...
                  return alpha ? str : str.slice(0, str.length - 1);
                }
              }) + '</span>' : '')+
            '</span>';
          }
    ...
    endQuiz : function (type) {
      ...
      if (storageOK && Genki.active.exercise.length > 0 && !/appendix|study-tools/.test(Genki.active.exercise[0])) {
        var lesson = Genki.active.exercise[0],
            lessonsResults = JSON.parse(localStorage.QuartetResults);
        ...
        localStorage.QuartetResults = JSON.stringify(lessonsResults);
          ...
          if (++Genki.dataBackupReminderCount >= 10) {
            ...
            setTimeout(function() {
              GenkiModal.open({
                ...
                '<div class="center">'+
                  '<a id="save-exercise-data" class="button" download="Quartet Exercise Score Data.txt" href="data:text/plain;charset=utf-8,' + (storageOK && localStorage.QuartetResults ? encodeURIComponent(localStorage.QuartetResults.replace(/\n/g, '\r\n')) : '') + '"><i class="fa">&#xf019;</i><span class="en">Save Data</span><span class="ja">セーブする</span></a><br><br>'+
                  ...
                  '<div>'+
                    '<div class="donate-box clear">'+
                      ...
                      '<div class="donate-text">If Quartet Study Resources has helped you with your studies, please also consider <a href="' + getPaths() + 'donate/' + Genki.local + '" target="_blank">making a donation <i class="fa">&#xf08e;</i></a> if you can, to help support the continued development and maintenance of these resources. Your support is greatly appreciated!</div>'+
                    '</div>'+
                  '</div>'+
                '</div>',
                zIndex : 'low',
    ...
    reset : function (skipModal) {
      if (window.JSON) {
        ...
        // standard quizzes
          ...
          // reset script highlighting for exercises that have audio and scripts
          var scripts = document.querySelectorAll('.chokai-script');
          
          if (scripts) {
            for (var i = 0, j = scripts.length, audio; i < j; i++) {
              audio = document.getElementById(scripts[i].id.replace('script-', ''));
          ...
          timer.addEventListener('secondsUpdated', function (e) {
            ...
            if (timeString == '00:00:00') {
              ...
              if (!Genki.local && Genki.canNotify && Notification.permission == 'granted') {
                var notif = new Notification(document.title.replace(/ \| Quartet Study Resources.*$/, ''), {
    ...
    // underlines text in exercises where you need to underline parts of a sentence
    underline : function (caller, answer) {
      var input = caller.parentNode.getElementsByTagName('INPUT')[0],
          parts = caller.parentNode.querySelectorAll('span:not(.writing-zone)'), i = 0, j = parts.length, next;
      
      // reset class and remove the underline for each part of the sentence
      for (; i < j; i++) {
        parts[i].className = '';
        parts[i].dataset.mistakes = '0';
      }
      
      // apply the underline for the caller and the siblings next to it
      caller.className += 'ul-color inline';
      caller.dataset.mistakes = answer == '0' ? 1 : 0;
      next = caller.nextSibling;
      
      while (next) {
        next.className += 'ul-color inline';
        next.dataset.mistakes = answer == '0' ? 1 : 0;
          ...
          callback : function () {
            ...
            // standard written quizzes
              ...
              for (; i < j; i++) {
                ...
                if (correct) {
                  var parent = input[i].parentNode.parentNode;
                  
                  if (parent && /underline-field-answer/.test(parent.className)) {
                    parent.parentNode.className += ' answer-correct';
                  } else {
    ...
    toggle : {
      ...
      // toggles display of the script in listening comprehension exercises
      chokaiScript : function (scriptId) {
        var script = document.getElementById('script-' + scriptId);
        
        if (script) {
          script.style.display = /none/.test(script.style.display) ? '' : 'none';
        }
      }
    },
    ...
    create : {
      ...
      exerciseList : function () {
        ...
        if (Genki.exercises) {
          ...
          if (storageOK && !localStorage.QuartetResults) {
            localStorage.QuartetResults = JSON.stringify({});
          }

          // loop over all the exercises and place them into their respective lesson group
          for (; i < j; i++) {
            ...
            var resultsStorage = JSON.parse(localStorage.QuartetResults),
    ...
    // highlights chokai script lines when playing audio
    chokai : {
      time : {},
      lines : {},
      highlight : function (id, time) {
        var audio = document.getElementById(id);
        
        Genki.chokai.lines[id] = document.querySelectorAll('#script-' + id + ' .line');
        Genki.chokai.time[id] = time;

        if (audio) {
          audio.addEventListener('timeupdate', function () {
            // apply the highlighted class to lines when a time range is matched
            // the time array indices correspond to those in Genki.chokai.lines
            for (var i = 0, j = time.length, active; i < j; i++) {
              if (this.currentTime >= time[i][0] && this.currentTime <= time[i][1] && Genki.chokai.lines[id][i]) {
                if (!/highlighted/.test(Genki.chokai.lines[id][i].className)) {
                  var active = document.querySelector('.line.highlighted');
                  if (active) {
                    active.className = active.className.replace('highlighted', '');
                  }

                  Genki.chokai.lines[id][i].className += ' highlighted';
                }
              }
            }

            // remove highlight class if the time range is less than or greater than the first or last time stamp
            if (this.currentTime < time[0][0] || this.currentTime > time[time.length - 1][1]) {
              var active = document.querySelector('.line.highlighted');
              if (active) {
                active.className = active.className.replace('highlighted', '');
              }
    ...
    getGrammarPoint : function (caller, id) {
      ...
      Get(caller.href, function (data) {
        var zone = document.getElementById('appendix-tool'),
            subId = id.replace(/l\d+-p\d+-(.*)/, '$1'),
            grammar = data.match(new RegExp('(<h3 id="' + id.replace(/-\d+$/, '') + '"[\\s\\S]*?<\/table><br>)', 'm')), // should return h3 title and table right below it
        ...
        if (grammar && grammar[0]) {
          if (zone) {
            ...
            zone.innerHTML = grammar[0].replace(/\d+\. /, '').replace(/href="#(l\d+-p\d+)"/g, 'onclick="Genki.getGrammarPoint(this, \'$1\'); return false;" target="_blank" href="' + url + '#$1"');
            zone.className = 'grammar-index';
            
            if (!/l/.test(subId)) {
              document.getElementById('genki-modal-content').scrollTop = document.getElementById(id).offsetTop + 20;
            }
          }
        } else if (zone) {
    ...
    randomExercise : function () {
      ...
      else if (storageOK && localStorage.genkiRandomExercise == 'completed' && localStorage.QuartetResults && JSON.parse(localStorage.QuartetResults)) {
        var Results = JSON.parse(localStorage.QuartetResults);
      ...
      else if (storageOK && localStorage.genkiRandomExercise == 'custom' && QuartetRandomList.length) {
        var list = QuartetRandomList;
      } 
    ...
    init : function () {
      if (Genki.exercises) {
        ...
        if (Genki.active.exercise) {
          ...
          result.insertAdjacentHTML('beforebegin', '<h2 id="exercise-title" class="center" ' + (Genki.active.exercise[2] ? 'data-page="Quartet ' +
              (+Genki.active.exercise[0].replace(/lesson-(\d+).*/, '$1') < 7 ? 'I' : 'II') +
              (/workbook-|wb-/.test(Genki.active.exercise[0]) ? ' Workbook' : '') + ': ' + Genki.active.exercise[2] + '"' : '') + '>' + (
            lesson == 'appendix' ? '巻末' :

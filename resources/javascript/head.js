...
(function (window, document) {
  ...
  window.QuartetRandomList = storageOK && localStorage.quartetRandomList ? JSON.parse(localStorage.quartetRandomList) : [];
  ...
  window.GenkiSettings = {
    manager : function () {
      ...
      var fontSize = +localStorage.genkiFontSize || 100,
          ...
          quartetTheme = localStorage.quartetTheme || 'blue',
      ...
      GenkiModal.open({
        ...
        content : '<p><span class="en">You can manage your settings for Quartet Study Resources in this window.<br>※ Please note that all data is saved locally to the browser, so clearing your browser\'s cache will reset your settings.</span><span class="ja">ここでQuartet Study Resourcesの設定が変更できます。<br>※ウェブサイトの設定はブラウザーで保存していますのでキャッシュをクリアすると失います。注意してください。</span></p>'+
        ...
        '<ul class="genki-settings-list">'+
          ...
          '<li>'+
            ...
            '<select id="settings-theme" onchange="GenkiSettings.updateTheme(this);">'+
              '<option value="blue"' + ( quartetTheme == 'blue' ? ' selected' : '' ) + '>' + (GenkiLang == 'ja' ? '青い色' : 'Blue') + '</option>'+
              '<option value="green"' + ( quartetTheme == 'green' ? ' selected' : '' ) + '>' + (GenkiLang == 'ja' ? '緑色' : 'Green') + '</option>'+
              '<option value="cerise"' + ( quartetTheme == 'cerise' ? ' selected' : '' ) + '>' + (GenkiLang == 'ja' ? 'チェリーピンク' : 'Cerise') + '</option>'+
              '<option value="orange"' + ( quartetTheme == 'orange' ? ' selected' : '' ) + '>' + (GenkiLang == 'ja' ? 'オレンジ' : 'Orange') + '</option>'+
            '</select>'+
          '</li>'+
        ...
        '<ul class="genki-settings-list">'+
          ...
          '<li>'+
            ...
            '</span>'+
            '<a id="settings-save-exercise-data" class="button" download="Quartet Exercise Score Data.txt" href="data:text/plain;charset=utf-8,' + (storageOK && localStorage.QuartetResults ? encodeURIComponent(localStorage.QuartetResults.replace(/\n/g, '\r\n')) : '') + '"><i class="fa">&#xf019;</i><span class="en">Save</span><span class="ja">セーブする</span></a>'+
    ...
    loadExerciseData : function (input) {
      ...
      reader.onload = function (e) {
        if (/"3rd"\:|"2nd"\:/.test(e.target.result)) return alert(GenkiLang == 'ja' ? 'このデータはGenki Study Resources専用です。Quartet Study Resourcesのデータを選択してください。' : 'This exercise data is only for use with Genki Study Resources. Please select your Quartet Exercise Data file to load your scores.');
        
        if (/"\:\d+/.test(e.target.result)) {
          localStorage.QuartetResults = e.target.result;
          ...
          alert(GenkiLang == 'ja' ? 'エラーが発生しました。やり直してみてください。またエラーが発生しましたらGitHubで連絡してください：https://github.com/SethClydesdale/quartet-study-resources/issues' : 'There was an error loading the exercise score data. Please try again or open a new issue on GitHub for help: https://github.com/SethClydesdale/quartet-study-resources/issues');
        }
      };
    ...
    updateTheme : function (caller) {
      ...
      // apply new theme
      if (caller.value != 'blue') {
        var theme = document.createElement('LINK');
        theme.id = 'website-theme';
        theme.href = getPaths() + 'resources/css/themes/theme-' + caller.value + '.min.css';
      ...
      // save preference to cache
      if (storageOK) {
        localStorage.quartetTheme = caller.value;
      }
    },
    ...
    randomExerciseList : function () {
      ...
      var parseExercises = function () {
        ...
        for (var list = '', selected = '', i = 0, j = GenkiExercises.length, data, checked, item; i < j; i++) {
          checked = QuartetRandomList.indexOf(GenkiExercises[i]) != -1;
          ...
          if (!/study-tools|appendix/i.test(data[0])) {
            item = '<div data-lesson="' + data[0].replace(/.*?lesson-(\d+).*/, 'L$1') + '" data-path="' + data[0] + '" data-name="' + data[1] + '">'+
                '<input class="def-selector genki_input_hidden" type="checkbox" onchange="GenkiSettings.updateRandomList(this, \'' + GenkiExercises[i] + '\');"' + (QuartetRandomList.indexOf(GenkiExercises[i]) != -1 ? ' checked' : '') + '>'+
      ...
      if (!window.GenkiExercises) {
        ...
        exercises.src = getPaths() + 'resources/javascript/exercises/quartet.min.js';
    ...
    updateRandomList : function (caller, data) {
      ...
      // remove exercise from the random list
        ...
        QuartetRandomList.splice(QuartetRandomList.indexOf(data), 1);
      ...
      localStorage.quartetRandomList = JSON.stringify(QuartetRandomList);
    },
  ...
  if (storageOK) {
    if (localStorage.quartetTheme && localStorage.quartetTheme != 'blue') {
      document.write('<link id="website-theme" href="' + getPaths() + 'resources/css/themes/theme-' + localStorage.quartetTheme + '.min.css" rel="stylesheet">');
    }
  }
  ...
  // If cookies are blocked, Quartet Study Resources will run in limited mode. Settings are not remembered in this mode and certain features, such as dark mode, are unavailable.
  if (!navigator.cookieEnabled) {
    console.warn('Cookies are not available either due to host or browser settings. Quartet Study Resources will function in limited mode where settings are not remembered and certain features are unavailable. This issue can commonly be resolved by enabling third-party cookies. Please see the following page for help.\nhttps://sethclydesdale.github.io/quartet-study-resources/help/stuck-loading/\n\nIf the issue still occurs after enabling third-party cookies, please contact the developer for further assistance.\nhttps://github.com/SethClydesdale/quartet-study-resources/issues');
  }
